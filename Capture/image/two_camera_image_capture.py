...
import os
import random
from send_image_to_rlef import SendImageToRLEF
...
class TwoCameraImageCapture:
    ...
    def capture_images(self, model_name="", label="", tags=[], initial_confidence_score=100, prediction="initial", metadata="", delete_image_after_use=False):
        """
        Captures images from multiple cameras simultaneously and saves them to the specified destination folders based on camera index.
        
        Parameters:
        model_name (str): The name of the RLEF model.
        label (str): The label for the images.
        tags (list): List of tags for the images corresponding to each camera.
        initial_confidence_score (int): The initial confidence score for the images.
        prediction (str): The prediction for the images.
        metadata (str): Additional metadata for the images.
        delete_image_after_use (bool): Whether to delete the images after sending them to RLEF.
        """
        ...
        while True:
            ...
            if key == ord('c'):
                for i, frame in enumerate(frames):
                    image_path = os.path.join(camera_folders[i], f"camera_{i}_image_{counter}.png")
...
if __name__ == "__main__":
    ...
    if rlef_model_id == "":
        two_camera_capture.capture_images()
    else:
        model_name = "Item In Hand Classification"
        label = 'initial'
        tags = ['top_left_shelf', 'top_right_shelf']
        two_camera_capture.capture_images(model_name, label, tags)
